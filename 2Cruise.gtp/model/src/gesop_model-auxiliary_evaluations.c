 ...
 *   Constants -
 *--------------*/
static DLL_Group_Item Algebraic_Function_Names[102] =
	{
	...
	{"AUX_LCI_kep_i",			-1, "Inclination in LCI",								-1, "Radian",			-1,"",-1	},	//33
	{"AUX_LCI_kep_omega",		-1, "Argument of periapsis in LCI",						-1, "Radian",			-1,"",-1	},	//34
	{"AUX_LCI_kep_RAAN",		-1, "RAAN in LCI",										-1, "Radian",			-1,"",-1	},	//35
	{"AUX_LCI_kep_nu",			-1, "True anomaly in LCI",								-1, "Radian",			-1,"",-1	},	//36
	{"AUX_SEL_kep_i",			-1, "Inclination in SEL",								-1, "Radian",			-1,"",-1	},	//37
	{"AUX_SEL_kep_omega",		-1, "Argument of periapsis in SEL",						-1, "Radian",			-1,"",-1	},	//38
	{"AUX_SEL_kep_RAAN",		-1, "RAAN in SEL",										-1, "Radian",			-1,"",-1	},	//39
	...
	{"AUX_Moon_nu_ECI",			-1,	"Lunar true anomaly in ECI",						-1, "Radian",			-1,"",-1	},	//75
	{"AUX_delta_V",				-1,	"Delta V",											-1, "Meter/Second",		-1,"",-1	},	//76
	{"AUX_periapsis",			-1,	"Periapsis distance in ECI",						-1, "Meter",			-1,"",-1	},	//77
	{"AUX_energy_used",			-1,	"Energy used by thrusters",							-1, "Joule",			-1,"",-1	},	//78
	{"AUX_energy_generated",	-1,	"Energy generated by solar panels",					-1, "Joule",			-1,"",-1	},	//79
	{"AUX_oblateness_higher",	-1,	"Higher order Earth oblateness",					-1, "Meter/Second**2",	-1,"",-1	},	//80
	{"AUX_energy_integral",		-1,	"Integral of instantaneous power",					-1, "Joule",			-1,"",-1	},	//81
	{"AUX_energy_net",			-1,	"Difference of thruster and PV integral energies",	-1, "Joule",			-1,"",-1	},	//82
	{"AUX_oblateness_J2",		-1,	"Earth oblateness due to J2 harmonic",				-1, "Meter/Second**2",	-1,"",-1	},	//83
	{"AUX_julian_date",			-1,	"Julian Date",										-1, "Day",				-1,"",-1	},	//84
	{"IBC_p",					-1,	"Initial condition: p",								-1, "None",				-1,"",-1	},	//85
	{"IBC_f",					-1,	"Initial condition: f",								-1, "None",				-1,"",-1	},	//86
	{"IBC_g",					-1,	"Initial condition: g",								-1, "None",				-1,"",-1	},	//87
	{"IBC_h",					-1,	"Initial condition: h",								-1, "None",				-1,"",-1	},	//88
	{"IBC_k",					-1,	"Initial condition: k",								-1, "None",				-1,"",-1	},	//89
	{"IBC_L",					-1,	"Initial condition: L",								-1, "None",				-1,"",-1	},	//90
	{"IBC_t",					-1,	"Initial condition: t",								-1, "None",				-1,"",-1	},	//91
	{"IBC_m",					-1,	"Initial condition: m",								-1, "None",				-1,"",-1	},	//92
	{"IBC_E",					-1,	"Initial condition: E",								-1, "None",				-1,"",-1	},	//93
	{"FBC_a",					-1,	"Final condition: a",								-1, "None",				-1,"",-1	},	//94
	{"FBC_e",					-1,	"Final condition: e",								-1, "None",				-1,"",-1	},	//95
	{"FBC_i",					-1,	"Final condition: i",								-1, "None",				-1,"",-1	},	//96
	{"FBC_o",					-1,	"Final condition: o",								-1, "None",				-1,"",-1	},	//97
	{"FBC_O",					-1,	"Final condition: O",								-1, "None",				-1,"",-1	},	//98
	{"FBC_v",					-1,	"Final condition: v",								-1, "None",				-1,"",-1	},	//99
	{"FBC_t",					-1,	"Final condition: t",								-1, "None",				-1,"",-1	},	//100
	{"FBC_m",					-1,	"Final condition: m",								-1, "None",				-1,"",-1	},	//101
};
...
printf("Algebraic Functions\n");_flushall();
	...
	*error = 0;
	...
	if(*phase < 3) {	// ECI
		...
		cart2mee(cart_inert_coords_ECI, gmue[EARTH], mee_coords_ECI);
	...
	limanglebetween(&phase_moon, 0, 2*PI);
...
//		break;
...

	{
		...
		// Calculate delta V
/* BIG TEMPORARY HACK HERE */
		acc_h = 0;
		// to calculate real acc_h (which is close to zero anyway) we need to run all the calculations from RHS again here. 
		// Eventually I should put them all in a separate function that can be called in RHS and here.
/* END BIG TEMPORARY HACK */
		omega = 1.0 + f*cos(L) + g*sin(L);
		Ldot = sqrt(p/g_mue)*( h*sin(L) - k*cos(L) )*acc_h/omega + sqrt(p*g_mue)*(omega/p)*(omega/p);	
		deltaV += thrust/x[7] * rpar[2]/Ldot * (*t-old_t);
		...
		thruster_energy += thruster_power * rpar[2]/Ldot * (*t-old_t);
		solar_energy += solar_power * rpar[2]/Ldot * (*t-old_t);
		battery_level = solar_energy - thruster_energy + 2.8*3.6e6;
		net_power = solar_power - thruster_power;
		net_energy += net_power * rpar[2]/Ldot * (*t-old_t);
	...
	outfun[33] =	kep_coords_LCI[3];
	outfun[34] =	kep_coords_LCI[4];
	outfun[35] =	kep_coords_LCI[5];
	outfun[36] =	kep_coords_LCI[6];

	outfun[37] =	kep_coords_SEL[3];		// a, e and ny in LCI and SEL frame identical!!
	outfun[38] =	kep_coords_SEL[4];
	outfun[39] =	kep_coords_SEL[5];
...
//	outfun[58] =	kep_coords_MOP[5];//
	...
	outfun[78] =	thruster_energy;
	outfun[79] =	solar_energy;
	outfun[80] =	dvec_abs(pertharm_rotrad_CS,3);
	outfun[81] =	net_energy;
	outfun[82] =	battery_level;
	outfun[83] =	dvec_abs(pertharm_rotrad2,3);
	outfun[84] =	julian_date;

	outfun[85] =	x[0]/34432189.0 - 1.0;
	outfun[86] =	x[1]/0.0057426 + 1.0;
	outfun[87] =	x[2]/0.469 - 1.0;
	outfun[88] =	x[3]/0.1917 - 1.0;
	outfun[89] =	x[4]/1.4337 + 1.0;
	outfun[90] =	x[5]/338.00745678 - 1.0;
	outfun[91] =	x[6]/107.8188 + 1.0;
	outfun[92] =	x[7]/190.247 - 1.0;
	outfun[93] =	x[8]/2.8/3.6e6 - 1.0;

	outfun[94] =	kep_coords_ECI[1]/268406334 - 1.0;
	outfun[95] =	kep_coords_ECI[2]/0.0828 - 1.0;
	outfun[96] =	kep_coords_ECI[3]*R2D/20.382 - 1.0;
	outfun[97] =	kep_coords_ECI[4]*R2D/208.745 - 1.0;
	outfun[98] =	kep_coords_ECI[5]*R2D/329.162 - 1.0;
	outfun[99] =	kep_coords_ECI[6]*R2D/52.667 - 1.0;
	outfun[100] =	x[6]/1652.9534 - 1.0;
	outfun[101] =	x[7]/181.98 - 1.0;
