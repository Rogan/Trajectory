...
const double deltaL[1] = {
	2592000,
}; 	// phase duration (seconds) = 30 days
// Needed to normalise the timescale within each phase

/* --- Initial guesses for initial state --- */


const double Ini_p = 266942539.0;	// [m] a(1-e²)
const double Ini_f = -0.061479332;			// e*cos(w+O)
const double Ini_g = -0.009796775;			// e*sin(w+O)
const double Ini_h = 0.156181446;			// tan(i/2)*cos(O)
const double Ini_k = -0.091301638;				// tan(i/2)*sin(O)
const double Ini_L = 10.18364712;				// w+O+v
const double Ini_t = 1652.9534;			// [days]
const double Ini_m = 181.98;			// [kg]
...
const double p_ub =  4.0e9;		// For circular orbit at lunar radius. Will not be greater unless hyperbolic.
...
const double L_lb = -2000.0;		// [rad]
const double L_ub = 20000.0;

const double t_lb = 0.0;		// [days]
const double t_ub = 6000.0;
